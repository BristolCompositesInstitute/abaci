...
!         perturbation of the Jth component of the strain increment
!         array.
...
!         is calculated by taking one half the sum of the matrix and its
!         transpose.
...
!         the increment before UMAT is called, so that only the
!         corotational
...
!        (“USDFLD,” Section 25.2.39) or UEXPAN (“UEXPAN,” Section
!        25.2.20),
!        in which case the updated values are passed in. In all cases
!         STATEV must be returned as the values at the end of the
!         increment.
!         The size of the array is defined as described in 
!        “Allocating space” in “User subroutines: overview,” Section
!        25.1.1.
!
...
!      RPL
!         Volumetric heat generation per unit time at the end of the
!         increment
!         caused by mechanical working of the material.
!
!     DDSDDT(NTENS)
!          Variation of the stress increments with respect to the
!          temperature.
!
!     DRPLDE(NTENS)
...
!
!     PNEWDT
!        Ratio of suggested new time increment to the time increment
!        being
!        used (DTIME, see discussion later in this section). This
!        variable
!        allows you to provide input to the automatic time
!        incrementation
!        algorithms in ABAQUS/Standard (if automatic time incrementation
!        is chosen).
!        For a quasi-static procedure the automatic time stepping that
!        ABAQUS/Standard
!        uses, which is based on techniques for integrating standard
!        creep laws
!        (see “Quasi-static analysis,” Section 6.2.5), cannot be
!        controlled from within
!        the UMAT subroutine.
!        PNEWDT is set to a large value before each call to UMAT.
!        If PNEWDT is redefined to be less than 1.0, ABAQUS/Standard
!        must abandon the
!        time increment and attempt it again with a smaller time
!        increment. The
!        suggested new time increment provided to the automatic time
!        integration
!        algorithms is PNEWDT × DTIME, where the PNEWDT used is the
!        minimum value
!        for all calls to user subroutines that allow redefinition of
!        PNEWDT for this
!        iteration.
!        If PNEWDT is given a value that is greater than 1.0 for all
!        calls to user
!        subroutines for this iteration and the increment converges in
!        this iteration,
!        ABAQUS/Standard may increase the time increment. The suggested
!        new time increment
!        provided to the automatic time integration algorithms is PNEWDT
!        × DTIME, where
!        the PNEWDT used is the minimum value for all calls to user
!        subroutines for
!        this iteration.
!        If automatic time incrementation is not selected in the
!        analysis procedure,
!        values of PNEWDT that are greater than 1.0 will be ignored and
!        values of
...
!
!     STRAN(NTENS)
!         An array containing the total strains at the beginning of the
!         increment.
!         If thermal expansion is included in the same material
!         definition, the
!         strains passed into UMAT are the mechanical strains only (that
!         is, the
!         thermal strains computed based upon the thermal expansion
!         coefficient have
!         been subtracted from the total strains). These strains are
!         available for output
!         as the “elastic” strains.
!
!         In finite-strain problems the strain components have been
!         rotated to account for
!         rigid body motion in the increment before UMAT is called and
!         are approximations
!         to logarithmic strain.

!     DSTRAN(NTENS)
!         Array of strain increments. If thermal expansion is included
!         in the same
!         material definition, these are the mechanical strain
...
!        this point
!        at the start of the increment, based on the values read in at
!        the nodes.
!
!      DPRED
...
!
!      CMNAME
!        User-defined material name, left justified. Some internal
!        material models are given names starting with the “ABQ_”
!        character string. To avoid conflict, you should not use “ABQ_”
...
!         associated with
!         this material type (defined as described in “Allocating space”
!         in “User
...
!         the current
!         coordinates if geometric nonlinearity is accounted for during
!         the step
!         (see “Procedures: overview,” Section 6.1.1); otherwise, the
!         array contains
!         the original coordinates of the point.
!
!     DROT(3,3)
!          Rotation increment matrix. This matrix represents the
!          increment of rigid
!          body rotation of the basis system in which the components of
!          stress
!          (STRESS) and strain (STRAN) are stored. It is provided so
!          that vector- or
!          tensor-valued state variables can be rotated appropriately in
!          this subroutine:
!          stress and strain components are already rotated by this
!          amount before UMAT
!          is called. This matrix is passed in as a unit matrix for
!          small-displacement
!          analysis and for large-displacement analysis if the basis
!          system for the
!          material point rotates with the material (as in a shell
!          element or when a
...
!          line across
!          an element for a first-order element; it is half of the same
!          typical length
!          for a second-order element. For beams and trusses it is a
!          characteristic length
!          along the element axis. For membranes and shells it is a
!          characteristic length
!          in the reference surface. For axisymmetric elements it is a
!          characteristic length
!          in the  plane only. For cohesive elements it is equal to the
!          constitutive
!          thickness.
!
!      DFGRD0(3,3)
...
!          the increment.
!          See the discussion regarding the availability of the
...
!            increment.
!           The components of this array are set to zero if nonlinear
!           geometric effects
!           are not included in the step definition associated with this
!           increment. See
!           the discussion regarding the availability of the deformation
!           gradient for
!           various element types.
!
!      NOEL
